...
export default function App() {
  ...
  return <main className="app-shell">
    ...
    <section className="suggestions-panel"><div className="suggestions-heading"><div><p className="eyebrow">PARA FACILITAR</p><h2>Adicione produtos rápidos</h2><p className="muted">Toque em um produto para adicioná-lo à lista. Ele desaparece daqui depois de adicionado.</p></div></div><div className="suggestion-groups">{SUGGESTIONS.map((group) => { const availableItems = group.items.filter(([itemName]) => !activeItems.some((item) => item.name.toLocaleLowerCase('pt-BR') === itemName.toLocaleLowerCase('pt-BR'))); if (!availableItems.length) return null; return <div className="suggestion-group" key={group.id}><h3><span>{group.emoji}</span>{group.label}</h3><div className="suggestion-list">{availableItems.map(([itemName, itemCategory]) => <button type="button" className="suggestion-chip" key={itemName} onClick={() => addSuggestion(itemName, itemCategory)}><Plus size={13} />{itemName}</button>)}</div></div>; })}</div></section>    <section className="card"><form className="add-form" onSubmit={addItem}><input value={name} onChange={(event) => setName(event.target.value)} placeholder="O que você precisa comprar?" aria-label="Nome do item" /><select value={category} onChange={(event) => setCategory(event.target.value)} aria-label="Categoria">{CATEGORIES.map((item) => <option value={item.id} key={item.id}>{item.emoji} {item.label}</option>)}</select><button className="primary-button" type="submit"><Plus size={18} /> Adicionar item</button></form>{grouped.length ? <div className="groups">{grouped.map((group) => <section key={group.id} className={group.items.every((item) => item.isChecked) ? 'category-section category-complete' : 'category-section'}><h2><span>{group.emoji}</span>{group.label}<small>{group.items.length}</small>{group.items.every((item) => item.isChecked) && <em>Concluída</em>}</h2><ul>{group.items.map((item) => <li key={item.id} className={item.isChecked ? 'checked' : ''}><button className="check" onClick={() => toggleItem(item)} aria-label={`Marcar ${item.name}`}><Check size={14} /></button><span>{item.name}</span><button className="delete" onClick={() => removeItem(item)} aria-label={`Excluir ${item.name}`}><Trash2 size={16} /></button></li>)}</ul></section>)}</div> : <div className="empty"><ShoppingCart size={34} /><h2>Sua lista está vazia</h2><p>Adicione seu primeiro item para começar.</p></div>}</section>
