...
export default function App() {
  ...
  useEffect(() => {
    if (!user || !db) {
      ...
      setActiveListId(savedActive && nextLists.some((list) => list.id === savedActive) ? savedActive : null);
      return undefined;
    }
    return onSnapshot(query(collection(db, 'users', user.uid, 'lists'), orderBy('createdAt', 'asc')), (snapshot) => {
      ...
      try { savedActive = localStorage.getItem(`${localKey}-active-list`); } catch {}
      setActiveListId((current) => {
        if (current && nextLists.some((list) => list.id === current)) return current;
        if (savedActive && nextLists.some((list) => list.id === savedActive)) return savedActive;
        return null;
      });
    });
  }, [user]);
